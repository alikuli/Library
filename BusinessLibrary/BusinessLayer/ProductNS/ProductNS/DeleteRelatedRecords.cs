...
namespace UowLibrary.ProductNS
{
    public partial class ProductBiz
    {
        ...
        private void deleteLikesUnlikes(Product p)
        {
            var lstLikesUnlikes = p.LikeUnlikes.ToList();

            if (lstLikesUnlikes.IsNullOrEmpty())
                return;

            foreach (var item in lstLikesUnlikes)
            {
                LikeUnlikeBiz.DeleteActuallyEngine(item);
            }
            lstLikesUnlikes.Clear();
        }
